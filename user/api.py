...
from photo_manager.serializers import MentionsSerializer
...
@swagger_auto_schema(
    ...
    responses={status.HTTP_200_OK: MentionsSerializer(many=True)})
@api_view(['GET'])
def get_partial_matched_names(request):
    ...
    return Response(data=MentionsSerializer(matched_users, many=True).data, status=status.HTTP_200_OK)
